...
void drawHeading(int hdg) {
    // Clear the top line
    oled.fillRect(0, 0, oledWidth, 8, 0);
    ...
    for (int i = 1; i < 4; i++) {
        oled.drawPixel(dotPos + dotPitch * i, 4, 1);
        oled.drawPixel(dotPos - dotPitch * i, 4, 1);
    }
    ...
    oled.fillRect(centerX-10, 0, 20, 8, 0);
...
void drawWing(int lpos, int width, int thickness) {
    ...
    oled.fillRect(lpos, y, width, thickness, 0);
    oled.drawRect(lpos, y, width, thickness, 1);
}
...
void drawNose(int height, int thickness) {
    ...
    for (int i = 0; i < thickness; i++) {
        oled.drawLine(centerX-1, y1, x2L, y2, 1);
        oled.drawLine(centerX, y1, x2R, y2, 1);
        y1++;
        x2L += 2; x2R -= 2;
    }
}
...
void transformDraw(float M[2][3], point* p1, point* p2) {
    ...
    transform(M, p1, &p1t);
    transform(M, p2, &p2t);
    oled.drawLine(p1t.x, p1t.y, p2t.x, p2t.y, 1);
}
